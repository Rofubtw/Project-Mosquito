...
namespace Extensions
{
	public static class GameObjectExt
	{
		...
		private static void CenterOnChildren(this GameObject aParent)
		{
			Vector3 defaultPos = aParent.transform.position;

			Vector3 center = aParent.transform.GetCenter();

			List<Transform> children = aParent.transform.GetChildrenTransforms();

			children.ForEach(a => a.transform.parent = null);

			aParent.transform.position = center;

			children.ForEach(a => a.transform.parent = aParent.transform);

			aParent.transform.position = defaultPos;
		}

		public static IEnumerable<Vector3> ToVector3(this IEnumerable<GameObject> gos) => gos.Select(a => a.transform.position);

		public static void SetActiveChildren(this GameObject aParent, bool isActive)
		{
